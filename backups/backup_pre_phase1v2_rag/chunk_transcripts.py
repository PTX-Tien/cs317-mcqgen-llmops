...
# ═══════════════════════════════════════════════════════════════════════════════
# PHASE 1: Semantic Chunking — cắt theo embedding similarity giữa câu liền kề
# Toggle: CHUNKING_MODE=semantic (env var)
# ═══════════════════════════════════════════════════════════════════════════════

# Lazy-load embedding model (chỉ load khi cần, tránh overhead cho legacy mode)
_emb_model = None


def _get_emb_model():
    """Load BGE-m3 embedding model (lazy, 1 lần duy nhất)."""
    global _emb_model
...
def _build_sentences_from_words(all_words: list[dict]) -> list[dict]:
    ...
    Mỗi sentence chứa:
      - text: nội dung câu
      - start: timestamp bắt đầu (giây)
      - end: timestamp kết thúc (giây)
      - word_start_idx: index bắt đầu trong all_words
      - word_end_idx: index kết thúc (exclusive) trong all_words

    Cắt câu dựa trên:
      1. Dấu câu: . ? !
      2. Pause dài (> 2 giây giữa 2 từ liền kề) — giảng viên tạm dừng = ý mới
    """
    if not all_words:
        return []
    ...
    for i, w in enumerate(all_words):
        ...
        is_long_pause = (
            i + 1 < len(all_words)
            and all_words[i + 1]["start"] - w["end"] > 2.0
        )

        if is_punctuation_end or is_long_pause or i == len(all_words) - 1:
            text = " ".join(cw["word"].strip() for cw in current_words)
            text = _deduplicate_text(text)
            if text and len(text.split()) >= 3:  # bỏ câu quá ngắn (< 3 từ)
                sentences.append({
                    "text": text,
                    "start": current_words[0]["start"],
                    "end": current_words[-1]["end"],
                    "word_start_idx": current_start_idx,
                    "word_end_idx": i + 1,
                })
            current_words = []
            current_start_idx = i + 1

    # Nếu còn words chưa kết thúc câu
    if current_words:
        text = " ".join(cw["word"].strip() for cw in current_words)
        text = _deduplicate_text(text)
...
def _compute_adjacent_similarities(sentences: list[dict]) -> list[float]:
    """
    Embed tất cả câu (batch) → tính cosine similarity giữa mỗi cặp liền kề.

    Returns: list[float] có length = len(sentences) - 1
      similarities[i] = cosine(embed(sentences[i]), embed(sentences[i+1]))
    """
...
def _find_breakpoints(
    ...
    """
    Tìm breakpoints — vị trí giữa 2 câu mà similarity thấp bất thường.

    Returns: list[int] — indices trong similarities[] nơi nên cắt.
      breakpoint i = cắt giữa sentence[i] và sentence[i+1]
    """
    if not similarities:
        return []

    # Dynamic threshold: percentile-based, nhưng không thấp hơn min_similarity
    threshold = max(
...
def _group_sentences_to_chunks(
    ...
    Sau đó post-process: merge chunk nhỏ, split chunk lớn.
    """
    ...
    # ─── Bước 2: Merge groups quá nhỏ (< min_chunk_words) ─────────
    merged_groups: list[list[dict]] = []
    for group in groups:
        wc = sum(len(s["text"].split()) for s in group)
        if merged_groups and wc < min_chunk_words:
            # Merge vào group trước (gần hơn về vị trí)
            merged_groups[-1].extend(group)
        else:
            merged_groups.append(group)

    # ─── Bước 3: Split groups quá lớn (> max_chunk_words) ──────────
    final_groups: list[list[dict]] = []
    for group in merged_groups:
        wc = sum(len(s["text"].split()) for s in group)
        if wc <= max_chunk_words or len(group) <= 1:
            final_groups.append(group)
        else:
            # Tìm điểm similarity thấp nhất bên trong group để split
            # Cần tính lại global sentence indices
            split_groups = _split_large_group(group, sentences, similarities, max_chunk_words)
            final_groups.extend(split_groups)

    # ─── Bước 4: Build chunk dicts ─────────────────────────────────
    video_meta = VIDEO_META_MAP.get((chapter_id, int(video_sub)), {})
    youtube_url = video_meta.get("url", "")
    slide_file = video_meta.get("slide_file", "")
    slide_start_page = video_meta.get("slide_start_page", 0)
    ...
    chunks = []
    for seq, group in enumerate(final_groups, 1):
...
def _split_large_group(
    ...
    """
    Split 1 group quá lớn thành 2+ groups tại điểm similarity thấp nhất bên trong.
    """
    if len(group) <= 1:
        return [group]

    # Tìm global indices của các sentences trong group
    # Dựa vào object identity để match
    group_global_indices = []
    for s in group:
        for gi, gs in enumerate(all_sentences):
            if gs is s:
                group_global_indices.append(gi)
                break

    # Tìm internal similarity thấp nhất
    best_split = None
    best_sim = float("inf")
    for local_i in range(len(group) - 1):
        gi = group_global_indices[local_i]
        if gi < len(similarities):
            if similarities[gi] < best_sim:
                best_sim = similarities[gi]
                best_split = local_i + 1
    ...
    for sub in [left, right]:
        wc = sum(len(s["text"].split()) for s in sub)
        if wc > max_words and len(sub) > 1:
            results.extend(_split_large_group(sub, all_sentences, similarities, max_words))
        else:
            results.append(sub)
    return results
...
def _chunk_segments_semantic(
    ...
    """
    Semantic chunking: cắt transcript dựa trên embedding similarity giữa câu liền kề.

    Thuật toán:
      1. Ghép words → sentences (dấu câu + pause)
      2. Batch embed sentences (BGE-m3)
      3. Tính cosine similarity giữa mỗi cặp câu liền kề
      4. Tìm breakpoints (similarity < percentile threshold)
      5. Nhóm sentences → chunks
      6. Post-process: merge nhỏ, split lớn
    """
    # Bước 1: words → sentences
    sentences = _build_sentences_from_words(all_words)

    if len(sentences) <= 1:
        # Quá ít câu → fallback legacy
    ...
    # Bước 5-6: nhóm + post-process → chunks
    ...
    if chunks:
        ...
        print(
            f"     [semantic] {len(sentences)} sentences → {len(chunks)} chunks | "
            f"avg={sum(wcs)/len(wcs):.0f}w | "
            f"range=[{min(wcs)},{max(wcs)}]w | "
            f"breakpoints={len(breakpoints)}"
        )
    else:
        print(f"     [semantic] {len(sentences)} sentences → 0 chunks (fallback to legacy)")
        return _chunk_segments(
            all_words, chapter_id, video_sub, source_file, topics, chapter_title
        )
...
def run_chunking() -> list[dict]:
    ...
    Toggle:
      CHUNKING_MODE=semantic  → semantic chunking (Phase 1)
      CHUNKING_MODE=legacy    → word-count chunking (mặc định, hành vi cũ)
    """
    ...
    if all_chunks:
        ...
        # Thêm stats cho semantic mode
        if chunking_mode == "semantic":
            semantic_count = sum(1 for c in all_chunks if c.get("chunking_method") == "semantic")
            legacy_count = len(all_chunks) - semantic_count
            print(f"   semantic chunks: {semantic_count} | legacy fallback: {legacy_count}")
